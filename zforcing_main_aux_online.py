...
filename = args.env_name + '_model/zforce_reacher_decode_state_lr'+ str(args.lr) + '_bwd_weight_' + str(args.bwd_weight) + '_aux_w_' + str(args.aux_weight_start) + '_kld_w_' + str(args.kld_weight_start) + '_' + str(random.randint(1,500))
...
for iteration in count(1):
    ...
    log_line ='Iteration: %d, All loss is %.3f , foward loss is %.3f, backward loss is %.3f, aux loss is %.3f, kld is %.3f, l2 loss is %.3f' % (
            iteration,
            all_loss.item(),
            fwd_nll.item(),
            bwd_nll.item(),
